...
void Fit(const string infile, bool save = false, bool rebin = false, int weeks = 1, int n_tau_short = 4)
{   
    ...
    newfile.open(infile.c_str(),ios::in); 
    ...
    const int N_muons = data.size();    // number of events (mu+ and mu-) in one week
